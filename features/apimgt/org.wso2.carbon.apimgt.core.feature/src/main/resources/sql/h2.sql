...
CREATE TABLE IF NOT EXISTS AM_VHOST (
            ID INTEGER AUTO_INCREMENT,
            ...
            DESCRIPTION VARCHAR(256) NULL,
            PRIMARY KEY(ID),
            UNIQUE (TENANT_DOMAIN, NAME),
            UNIQUE (URL)
);

CREATE TABLE IF NOT EXISTS AM_VHOST_GW_ENV_MAPPING (
            VHOST_ID INTEGER,
            DEPLOYMENT_NAME VARCHAR(256)
            FOREIGN KEY (VHOST_ID) REFERENCES AM_VHOST(ID) ON UPDATE CASCADE ON DELETE CASCADE
);
